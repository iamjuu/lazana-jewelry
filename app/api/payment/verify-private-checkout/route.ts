...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if session enquiry already exists for this session
    const existingEnquiryCheck = await SessionEnquiry.findOne({
      paymentRef: sessionId,
    });
    const isNewBooking = !existingEnquiryCheck;
    ...
    if (!metadata) {
      ...
      return NextResponse.json(
        {
          success: false,
          message: "Invalid session metadata: metadata is missing",
        },
        { status: 400 },
      );
    }
      ...
      return NextResponse.json(
        {
          success: false,
          message:
            "Invalid session metadata: missing required fields (slotId or sessionId+enquiryId)",
          metadata: metadata,
        },
        { status: 400 },
      );
    ...
    // Prepare session enquiry data
    const bookingAmount = session.amount_total
      ? Math.round(session.amount_total / 100)
      : (privateSession.price ?? 0); // Convert from cents

    // We'll create/update the SessionEnquiry later in the flow
    if (isNewBooking) {
    ...
    const customerEmail =
      ...
      "";
    const customerName =
      session.customer_details?.name || user.name || "Customer";
    ...
    if (enquiryId) {
      ...
      if (existingEnquiry) {
        ...
        existingEnquiry.userId = String(user._id);
        existingEnquiry.amount = bookingAmount;
        existingEnquiry.paymentProvider = "stripe";
        existingEnquiry.paymentRef = sessionId;
        existingEnquiry.paymentStatus = "paid";
        ...
        try {
          enquiry = await SessionEnquiry.create({
            ...
            comment: `Private Session booking - Payment confirmed. Amount: INR ${bookingAmount.toFixed(2)}`,
            status: "completed",
            sessionType: "private",
            sessionId: sessionIdToUse,
          ...
          console.log(
            "✅ Created SessionEnquiry for private session booking:",
            enquiry._id,
          );
        } catch (error) {
    ...
    if (customerEmail) {
      sendPrivateSessionConfirmationToUser({
        ...
        console.error(
          "Failed to send private session confirmation email to customer:",
          error,
        );
      });
    }
